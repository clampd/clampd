 ...
 *
 * Use `clampd.agent(id, async () => { ... })` to enter a delegation
 * scope — every `clampd.guard()`-wrapped call inside that scope
 * automatically inherits the chain. You DO NOT need to call
 * `withDelegation()` / `getDelegation()` directly; those are SDK
