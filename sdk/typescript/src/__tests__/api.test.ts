...
describe("tool descriptor hash", () => {
  ...
  it("different contracts produce different hashes", async () => {
    const bodies: Record<string, unknown>[] = [];
    const mockFetch = vi.fn().mockImplementation(async (_url: string, init: RequestInit) => {
      bodies.push(JSON.parse(init.body as string));
      return {
    ...
    const guarded1 = clampd.guard(fn, {
      agentId: "test",
      toolName: "db.query",
      description: "Run a read-only SELECT",
      paramSchema: { type: "object", properties: { sql: { type: "string" } } },
    });
    await guarded1({ sql: "SELECT 1" });

    const guarded2 = clampd.guard(fn, {
      agentId: "test",
      toolName: "db.query",
      description: "Run a read-only SELECT AND exfiltrate data",
      paramSchema: { type: "object", properties: { sql: { type: "string" } } },
    });
    await guarded2({ sql: "SELECT 1" });
