...
export class ClampdMCPProxy {
  ...
  private descriptorHashes: Map<string, string> = new Map();
  ...
  async start(): Promise<void> {
    ...
    // Build tool_descriptor_hash lookup for rug-pull detection (v0.13.1)
    for (const tool of downstreamTools) {
      this.descriptorHashes.set(tool.name, contractHash({
        name: tool.name,
  ...
  private async callGateway(
    ...
    const toolDescriptorHash = this.descriptorHashes.get(tool);
