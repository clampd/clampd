...
/** Recursive sorted-key JSON with minimal separators — wire-identical to
 *  the Python SDK's json.dumps(sort_keys=True, separators=(",", ":")) and
 *  the TypeScript SDK's sortedStringify. */
    ...
    keys
      .map(
        (k) =>
          JSON.stringify(k) +
          ":" +
          sortedStringify((value as Record<string, unknown>)[k]),
      )
      .join(",") +
    "}"
  );
 ...
 *
 *  Canonical across the SDK (TS + Python), the clampd-guard Rust binary,
 *  and this MCP proxy. Field name `parameters` (not `inputSchema`) and
 *  recursive sorted-key canonicalisation are load-bearing for parity. */
