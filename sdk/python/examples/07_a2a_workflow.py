...
Use ``with clampd.agent(id):`` to enter a delegation scope — every
``@clampd.guard()``-decorated function called inside that scope
automatically inherits the chain. You DO NOT need to call
``enter_delegation()`` / ``exit_delegation()`` directly; those are SDK
internals and are exported only for backward compat with code that
predates ``clampd.agent()``. Prefer ``clampd.agent()`` in all new code.
