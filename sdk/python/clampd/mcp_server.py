...
class ClampdMCPProxy:
    ...
    def __init__(
        ...
        *,
        ...
        self._descriptor_hashes: dict[str, str] = {}
    ...
    def _check_with_clampd(
        ...
        tool_hash = self._descriptor_hashes.get(tool_name)
        ...
        try:
            if self.dry_run:
                return clampd.verify(
                    ...
                    tool_descriptor_hash=tool_hash,
                )
            return clampd.proxy(
                tool=tool_name,
                params=arguments,
    ...
    async def run(self) -> None:
        ...
        async with stdio_client(downstream_params) as (ds_read, ds_write):
            async with ClientSession(ds_read, ds_write) as downstream:
                ...
                self._descriptor_hashes = {
                    t.name: contract_hash(
                        t.name,
                        t.description or "",
                        t.inputSchema or {},
                    )
                    for t in self._downstream_tools
                }
                logger.info(
