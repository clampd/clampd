...
class TestCrewAIGuard:
    ...
    def test_step_callback_allows_tool_call(self):
        ...
        step.tool_description = "web search"

        result = guard.step_callback(step)

        assert result is step
        call_kwargs = client.proxy.call_args.kwargs
        assert call_kwargs["tool"] == "search"
        assert call_kwargs["params"] == {"query": "hello"}
        assert call_kwargs["target_url"] == ""
        # v0.13.2 — SDK emits a descriptor hash on every guarded call
        assert isinstance(call_kwargs["tool_descriptor_hash"], str)
        assert len(call_kwargs["tool_descriptor_hash"]) == 64
    ...
    def test_wrap_tool_guards_run(self):
        ...
        call_kwargs = client.proxy.call_args.kwargs
        assert call_kwargs["tool"] == "calculator"
        assert call_kwargs["params"] == {"expression": "2+2"}
        assert call_kwargs["target_url"] == ""
        assert isinstance(call_kwargs["tool_descriptor_hash"], str)
        assert len(call_kwargs["tool_descriptor_hash"]) == 64
        assert result == "42"
