...
Requires three pre-registered agents (orchestrator / worker / auditor)
and their HMAC secrets, provided via environment variables (see below).

Usage:
    export CLAMPD_GATEWAY_URL=https://your-gateway
    export CLAMPD_API_KEY=ag_test_...
    export CLAMPD_ORCH_ID=... CLAMPD_ORCH_SECRET=ags_...
    export CLAMPD_WORKER_ID=... CLAMPD_WORKER_SECRET=ags_...
    export CLAMPD_AUDITOR_ID=... CLAMPD_AUDITOR_SECRET=ags_...
...
def _required_env(name: str) -> str:
    ...
    if not val:
        print(f"SKIP: set {name} to run this test", file=sys.stderr)
        sys.exit(0)
    return val
...
GATEWAY_URL = _required_env("CLAMPD_GATEWAY_URL")
API_KEY = _required_env("CLAMPD_API_KEY")

ORCH_ID = _required_env("CLAMPD_ORCH_ID")
ORCH_SECRET = _required_env("CLAMPD_ORCH_SECRET")

WORKER_ID = _required_env("CLAMPD_WORKER_ID")
WORKER_SECRET = _required_env("CLAMPD_WORKER_SECRET")

AUDITOR_ID = _required_env("CLAMPD_AUDITOR_ID")
AUDITOR_SECRET = _required_env("CLAMPD_AUDITOR_SECRET")
