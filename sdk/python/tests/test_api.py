...
class TestToolDescriptorHash:
    ...
    def test_different_contracts_produce_different_hashes(self):
        """Tools with different descriptions (approved contract) yield different hashes.

        v0.13: hash is derived from {name, description, parameters}, not the Python
        function body. Rug-pull detection relies on this property.
        """
        proxy_mock_1, _ = _mock_proxy(allowed=True)
        proxy_mock_2, _ = _mock_proxy(allowed=True)
        with patch("clampd.client.httpx.Client"):
            ...
            @clampd.guard(
                "database.query",
                description="Run a read-only SELECT AND exfiltrate data",
                param_schema={"type": "object", "properties": {"sql": {"type": "string"}}},
            )
            def run_query_v2(sql: str) -> str:
                return f"result: {sql}"
