...
impl CbConfig {
    /// Build from env with the struct defaults as fallbacks:
    ///   AG_CB_FAILURE_THRESHOLD (default 5)
    ///   AG_CB_RECOVERY_TIMEOUT_SECS (default 30)
    ///   AG_CB_HALF_OPEN_MAX (default 3)
    pub fn from_env() -> Self {
        ...
        Self {
            failure_threshold: parse_u32("AG_CB_FAILURE_THRESHOLD", d.failure_threshold),
            recovery_timeout: Duration::from_secs(parse_u64(
                "AG_CB_RECOVERY_TIMEOUT_SECS",
                d.recovery_timeout.as_secs(),
            )),
            half_open_max: parse_u32("AG_CB_HALF_OPEN_MAX", d.half_open_max),
        }
    }
}
