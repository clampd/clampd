...
pub async fn handle_proxy(
    ...
    if let Some(ref ctx) = delegation_ctx {
        ...
        if ctx.chain.len() > 1 {
            ...
            if !crate::delegation::verify_agent_exists(&state.redis_pool, caller).await {
                let reason = format!(
    ...
    let is_payment_tool = tool_name.starts_with("payment.") || tool_name.starts_with("billing.") || tool_name.starts_with("stripe.") || tool_name.starts_with("checkout.") || tool_name.starts_with("invoice.");
    let (payment_amount_cents, payment_recipient, payment_vendor_approved) = if is_payment_tool {
        ...
        let vendor_approved = if let Some(ref bounds) = agent_profile.boundaries {
            if bounds.approved_vendors.is_empty() {
                true // no whitelist = all approved
            } else {
                vendor.is_empty() || bounds.approved_vendors.iter().any(|v| v.to_lowercase() == vendor.to_lowercase())
            }
        } else {
    ...
    let (assessed_risk, classification, mut intent_labels, mut matched_rules, reasoning, intent_action, has_non_exemptable_block) =
