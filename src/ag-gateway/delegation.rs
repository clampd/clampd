...
pub fn validate_chain(chain: &[String]) -> Result<(), DelegationError> {
    ...
    for agent in chain {
        if !seen.insert(agent.to_lowercase()) {
...
/// Verify that an agent exists by checking its credential cache in Redis.
...
pub async fn verify_agent_exists(redis_pool: &Pool<RedisConnectionManager>, agent_id: &str) -> bool {
    let key = format!("ag:agent:cred:{}", agent_id);
    let mut conn = match redis_pool.get().await {
        Ok(c) => c,
        Err(_) => return false, // fail-closed: reject unknown agents if Redis unavailable
    };
...
mod tests {
    ...
    #[test]
    fn adversarial_tool_allowed_prefix_boundary() {
        let tools = vec!["db".to_string()];
        // "db" should match "db.query" via prefix
        assert!(is_tool_allowed(&tools, "db.query"), "Prefix match with dot");
        // But should NOT match "dba" (no dot)
        assert!(!is_tool_allowed(&tools, "dba"), "No dot = no prefix match");
        // And should NOT match "database.query"
        assert!(!is_tool_allowed(&tools, "database.query"), "Different prefix");
    }

    #[test]
    fn adversarial_tool_allowed_empty_list() {
        assert!(is_tool_allowed(&[], "anything"), "Empty list allows all tools");
    }

    #[test]
    fn adversarial_tool_allowed_exact_match() {
        let tools = vec!["shell.exec".to_string()];
        assert!(is_tool_allowed(&tools, "shell.exec"), "Exact match");
        assert!(is_tool_allowed(&tools, "shell.exec.run"), "Sub-tool matches via prefix");
        assert!(!is_tool_allowed(&tools, "shell.execute"), "Different suffix does not match");
    }
