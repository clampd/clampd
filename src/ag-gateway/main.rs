...
pub struct AppState {
    ...
    /// L1 in-process cache for delegation approvals + agent-existence checks.
    /// Same DashMap-with-deadline pattern as `baseline_cache`; cuts Redis
    /// round-trips on hot delegation paths (5s TTL).
    pub delegation_cache: Arc<crate::delegation::DelegationCache>,
...
async fn main() -> Result<()> {
    ...
    // Connect NATS + derive JetStream context. JetStream backs the durable
    // AGENTGUARD_CONTROL stream that delivers kill/revive events; the helper
    // idempotently declares the stream on startup so publisher/subscriber
    // boot order is irrelevant.
    let nats = connect_nats(&config.nats_url).await?;
    info!("Connected to NATS");
    let jetstream = async_nats::jetstream::new(nats.clone());
    ag_common::control_events::ensure_stream(&jetstream).await?;
    info!("JetStream AGENTGUARD_CONTROL stream ready");
    ...
    // Spawn JetStream listener for kill/revive broadcasts. Each gateway pod
    // gets its own durable consumer (pod-scoped name) so every pod receives
    // every event, with cursor replay on pod restart.
    kill_listener::spawn_kill_listener(jetstream.clone(), deny_set.clone(), redis_pool.clone()).await?;
    info!("Kill listener started (JetStream)");
    ...
    // Initialize circuit breakers for upstream gRPC services. Thresholds
    // env-tunable via AG_CB_FAILURE_THRESHOLD / AG_CB_RECOVERY_TIMEOUT_SECS /
    // AG_CB_HALF_OPEN_MAX.
    let circuit_breakers = Arc::new(CircuitBreakerManager::with_config(
        circuit_breaker::CbConfig::from_env(),
    ));
    ...
    let delegation_cache = Arc::new(delegation::DelegationCache::new(redis_pool.clone()));
    ...
    // v0.11.1 P4: trust-config cache for gating PII scanner when trust=Trusted.
    // Hydrated from Redis; NATS invalidation listener spawned via listener.
    let trust_cache = ag_common::trust_config::TrustConfigCache::new();
    if let Err(e) = trust_cache.refresh_from_redis(&redis_pool).await {
        tracing::warn!(error = %e, "initial trust config cache refresh failed");
    }
    trust_cache
        .clone()
        .spawn_invalidation_listener(nats.clone(), redis_pool.clone());
    ...
    // Build router. GATEWAY_MAX_CONCURRENCY caps in-flight requests so we
    // shed load with 503 under pressure instead of starving the Redis pool
    // and returning opaque 500s. 0 disables the limit.
    let max_concurrency: usize = std::env::var("GATEWAY_MAX_CONCURRENCY")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(1024);
    ...
    let proxy_routes = if max_concurrency > 0 {
        ...
        let shed_stack = ServiceBuilder::new()
            // Map LoadShed's Overloaded error → 503 so clients see clean backpressure.
            .layer(HandleErrorLayer::new(|_err: axum::BoxError| async {
                StatusCode::SERVICE_UNAVAILABLE
            }))
            .layer(tower::load_shed::LoadShedLayer::new())
            .layer(tower::limit::ConcurrencyLimitLayer::new(max_concurrency));
        proxy_routes.layer(shed_stack)
    } else {
        proxy_routes
    };
    ...
    // Metrics served on a separate port, intended to be bound to an internal
    // network (e.g. VPC-only) so Prometheus can scrape without exposing
    // service internals on the public gateway port.
    let metrics_app = axum::Router::new().route("/metrics", get(metrics_handler));
    let metrics_addr = format!("0.0.0.0:{}", config.metrics_port);
    let metrics_listener = TcpListener::bind(&metrics_addr).await?;
    info!("ag-gateway metrics on {} (internal only)", metrics_addr);
    tokio::spawn(async move {
