    ...
    let classify_result = if state.circuit_breakers.is_allowed("intent") {
        ...
        let result = client
            .classify_intent(ClassifyRequest {
                ...
                // /v1/scan-input is a content-only endpoint with no
                // target_url or boundary context - no trust signal possible.
                trust_level: ag_common::trust::TrustLevel::Unknown.to_string(),
                org_id: api_key_info.org_id.clone(),
                // Pass caller-provided scope so ag-intent can strip operational
                // fields when the input is JSON tool-call params. Empty string
                // preserves the legacy fallback (llm:input:prompt by default).
                resolved_scope: body.scope.clone().unwrap_or_default(),
            })
            .await;
        match &result {
...
pub async fn handle_scan_output(
    ...
    //
    // Note on weights: PII at 0.30 (not 0.85 as previously) so it compounds
    // with rule layer rather than hard-blocking on its own. Reasons:
    //   - A single email address in a tool response is a *signal*, not an
    //     attack on its own. Email-tool responses legitimately contain
    //     `from`/`to`/`recipient` addresses.
    //   - R203 (pii.toml) is the proper rule, tuned to 0.30 with URL
    //     exclusion and scope narrowing to `llm:output:*`/`comms:*`/`net:*`.
    //     Hardcoding 0.85 here bypassed R203's `exemptable=true` flag and
    //     R203's scope narrowing.
    //   - Secrets stay at 0.95 — an API key in a tool response is always
    //     a hard block, no scope makes that legitimate.
    let pii_found = detect_pii(&body.text);
    let secrets_found = detect_secrets(&body.text);

    let pii_risk = if pii_found.is_empty() { 0.0 } else { 0.30 };
    ...
    let classify_result = if state.circuit_breakers.is_allowed("intent") {
        ...
        let result = client
            .classify_intent(ClassifyRequest {
                ...
                trust_level: ag_common::trust::TrustLevel::Unknown.to_string(),
                org_id: api_key_info.org_id.clone(),
                // Pass scope when caller provides it so ag-intent can strip
                // operational fields (e.g. `comms:email` → strip to/cc/bcc/
                // from/reply_to before content rules scan). Empty string
                // preserves the legacy fallback path in ag-intent.
                resolved_scope: body.scope.clone().unwrap_or_default(),
            })
            .await;
        match &result {
...
mod tests {
    ...
    fn detect_secrets_finds_jwt() {
        let secrets = detect_secrets(
            "token: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.FAKE_TEST_SIGNATURE_NOT_A_REAL_JWT_PLACEHOLDER",
        );
    ...
    fn detect_secrets_finds_generic_api_key() {
        let secrets = detect_secrets("api_key: STRIPE_REDACTED_TEST_PLACEHOLDER");
    ...
    fn risk_scoring_pii_only() {
        // PII at 0.30 (signal, compounds with rules) per scan.rs production code.
        // R203 handles authoritative blocking with scope narrowing + exemption.
        let pii = detect_pii("SSN: 123-45-6789");
        let secrets = detect_secrets("SSN: 123-45-6789");
        let pii_risk = if pii.is_empty() { 0.0 } else { 0.30 };
        let secrets_risk = if secrets.is_empty() { 0.0 } else { 0.95 };
        let combined = 0.0_f64.max(pii_risk).max(secrets_risk);
        assert!((combined - 0.30).abs() < f64::EPSILON);
    }

    #[test]
    fn risk_scoring_secrets_dominates() {
        // Secrets stay at 0.95 (hard block) — an API key in a tool response
        // is always suspicious, no scope makes it legitimate.
