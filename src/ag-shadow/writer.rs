...
pub struct BatchWriter {
    /// None when CLICKHOUSE_ENABLED=false — push/flush become no-ops.
...
impl BatchWriter {
    ...
    /// Build a disabled writer — consumer can still drain NATS, writes are dropped.
    pub fn disabled() -> Self {
        Self {
            client: None,
            buffer: Vec::new(),
            max_batch_size: 0,
            max_buffer_age: Duration::from_secs(u64::MAX / 2),
            last_flush: Instant::now(),
            batch_start: None,
            last_metrics: None,
        }
