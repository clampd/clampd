...
async fn main() -> Result<()> {
    ...
    if leader_follower_enabled {
        ...
        let leader_redis = bb8::Pool::builder()
            .max_size(std::env::var("REDIS_POOL_MAX_SIZE").ok().and_then(|v| v.parse().ok()).unwrap_or(32))
...
async fn init_consumer_deps(
    ...
    // Connect to ClickHouse (optional — CLICKHOUSE_ENABLED=false or empty URL
    // turns shadow logging into NATS-drain-and-drop so JetStream doesn't back up).
    let ch_client = if config.clickhouse_enabled {
        let mut c = clickhouse::Client::default().with_url(&config.clickhouse_url);
        ...
        ensure_clickhouse_tables(&c).await?;
        info!("Connected to ClickHouse (tables verified)");
        Some(c)
    } else {
        warn!("ClickHouse disabled — shadow events will be consumed from NATS but not persisted (CLICKHOUSE_ENABLED=false or CLICKHOUSE_URL empty)");
        None
    };
    ...
    // Initialize quarantine writer (disabled no-op when ClickHouse is off).
    let quarantine_writer = if config.clickhouse_enabled {
        ...
        quarantine::QuarantineWriter::new(quarantine_ch)
    } else {
        info!("Quarantine writer disabled (ClickHouse off)");
        quarantine::QuarantineWriter::disabled()
    };
    ...
    let batch_writer = match ch_client {
        Some(c) => writer::BatchWriter::new(c, config.batch_size, config.flush_interval_secs),
        None => writer::BatchWriter::disabled(),
    };
