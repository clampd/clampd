...
/// Look up the hard-coded description for a built-in IDE tool.
///
/// Returns `None` for unknown / MCP tools — callers should fall back to
/// an empty description so the hash remains stable against any future
/// sync that also has no richer metadata.
pub fn known_tool_description(name: &str) -> Option<&'static str> {
    CLAUDE_CODE_TOOLS
        .iter()
        .chain(CURSOR_TOOLS.iter())
        .find(|(n, _)| *n == name)
        .map(|(_, d)| *d)
}
...
pub async fn run(target: &str) {
    ...
    for (name, description) in &tools {
        // Canonical hash — matches the SDKs and the hook-mode /v1/proxy
        // call. Parameters are unknown at sync time, so pass `{}`.
        let hash = contract_hash(name, description, &serde_json::json!({}));
