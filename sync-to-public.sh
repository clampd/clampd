...
# Resolution order: explicit env var > VERSION file in this directory.
# No hardcoded default — refuse to run unsigned if neither is set.
if [[ -n "${CLAMPD_VERSION:-}" ]]; then
    VERSION="$CLAMPD_VERSION"
elif [[ -f "$SCRIPT_DIR/VERSION" ]]; then
    VERSION="$(tr -d '[:space:]' < "$SCRIPT_DIR/VERSION")"
else
    echo "ERROR: set CLAMPD_VERSION or create $SCRIPT_DIR/VERSION" >&2
    exit 1
fi
...
# Secrets scan — clampd-specific high-confidence patterns only.
#
# This is a sanity check, not a full secrets audit (use gitleaks for that).
# It targets exactly the credential shapes that the clampd control plane
# issues: API keys (`ag_test_*`, `ag_live_*`) and agent HMAC secrets (`ags_*`).
# Generic patterns (PRIVATE KEY, password=, sk-*) were removed because
# scan.rs and mcp-proxy/mock-server intentionally contain such strings as
# *test payloads* for the detector itself — too many false positives.
#
# DO NOT add a broad `grep -v 'test|example'` substring filter — that's
# how the April 2026 leak slipped through. Every real secret literal here
# was inside a file or identifier containing one of those words.
SECRETS=$(grep -rEn \
    -e '(ag_test|ag_live)_[A-Za-z0-9_-]{20,}' \
    -e 'ags_[A-Za-z0-9_-]{20,}' \
    --include='*.rs' --include='*.toml' --include='*.yml' --include='*.yaml' \
    --include='*.json' --include='*.py' --include='*.ts' --include='*.tsx' \
    --include='*.js' --include='*.md' --include='*.sh' --include='*.env*' \
    --exclude-dir='node_modules' --exclude-dir='.git' --exclude-dir='target' \
    --exclude-dir='dist' --exclude-dir='__pycache__' --exclude-dir='.mypy_cache' \
    "$PUBLIC_ROOT" 2>/dev/null \
    | grep -vE '\b(fn|function|let|const)[[:space:]]+ags_[a-z_]+\(' \
    || true)
if [[ -n "$SECRETS" ]]; then
    echo "  ABORT: credential literals (ag_test_/ag_live_/ags_) found:"
    echo "$SECRETS"
    LEAKED=true
fi
