...
async fn main() -> anyhow::Result<()> {
    ...
    // are not needed (proxy reads the same Postgres).
    {
        ...
        // Set license status in Redis so the gateway license gate allows requests.
        // In non-shared mode, the heartbeat loop does this periodically.
        if let Ok(mut conn) = redis.get().await {
            let _: Result<(), _> = redis::cmd("SET")
                .arg("ag:license:status")
                .arg("valid")
                .query_async(&mut *conn)
                .await;
            info!("Set ag:license:status=valid in Redis (shared DB mode)");
        }
